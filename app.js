...
const defaultDoctors = [
  ...
  { name: '服部', request: '土日休日は2nd待機のみ希望\n5/15 不可' },
...
3. 2nd当番および備考:
   - 当直を担当する日は、2nd当番を兼任することはできません。
   - バックアップ要員としての2nd当番は、当直経験のバランスを考慮して配置してください。
4. 休日の特別ルール:
...
* 平日当番:
  ...
  - 2nd当番: 平日は ${symbolA} または ${symbolB} を中心にバックアップを構成。
* 土日休日当番:
  - 担当医師: ${symbolB}、${symbolA}、${symbolC}、${symbolD} の4名
  - 割合の目安: 4名で概ね均等
  - 連続アサイン: 土日などの連続した休日は、できる限り「同一人物が連続して担当」するように配置すること。${symbolD}は必ず連続させる。
  - 特殊制約: 土日休日に${symbolC}が当番の日には${symbolB}または${symbolA}を2ndとして割り当てる。

### 【出力フォーマット】
以下の構成で出力してください。
1. **月間カレンダー形式のシフト一覧** (日付、曜日、当直、日直、2nd、備考)
2. **各メンバーの合計担当回数集計表** (当直、日直、2ndのそれぞれの合計)
